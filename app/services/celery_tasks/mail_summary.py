...
@celery_app.task
def mail_weekly_po_summary():

    purchase_records = db.query(PurchaseOrder).all()

    if not purchase_records:
        return "No purchase_records found."

    wb = Workbook()
    current_ws = wb.active
    current_ws.title = "Purchase Order Summary"

    # Header
    current_ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=5)
    current_ws.cell(row=1, column=1).value = "All Warehouses - Purchase Order Summary"
    current_ws.cell(row=1, column=1).alignment = Alignment(horizontal="center")
    current_ws.append(["PO_ID", "Status","Total PO cost", "Supplier ID",  "Warehouse Id","Expected date", "Status updated at"])
    ...
    for item in purchase_records:
        current_ws.append([
            ...
            str(item.warehouse_id),
            str(item.expected_date),
            str(item.warehouse_id),
            str(item.status_updated_at),
        ])


    reports_dir = "Weekly_po_reports"
    os.makedirs(reports_dir, exist_ok=True)
    file_name = f"Weekly_po_summary_{uuid.uuid4()}.xlsx"
    file_path = os.path.join(reports_dir, file_name)
    wb.save(file_path)


    # email_obj = Email()
    # email_obj.send_email("Weekly PO Summary", "Weekly PO Summary","ys6244864@gmail.com",file_path)
